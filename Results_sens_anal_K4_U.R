...
p_k.gn <- ggplot(df.k_es.gn, aes(x=X2, y=X1)) + 
  geom_boxplot()+xlab("Gnedin's gamma")+ylab("K.est (K = 4)")
#p_k.gn
...
p_k.po <- ggplot(df.k_es.po, aes(x=X2, y=X1)) + 
  geom_boxplot()+xlab("tr Poisson's lambda")+ylab("K.est (K = 4)")
#p_k.po
...
p_prob.gn <- ggplot(df.prob.gn, aes(x=X2, y=X1)) + 
  geom_boxplot()+xlab("Gnedin's gamma")+ylab("P(K = 4 | A)")+geom_jitter(shape=16, position=position_jitter(0.2))
#p_prob.gn
...
p_prob.po <- ggplot(df.prob.po, aes(x=X2, y=X1)) + 
  geom_boxplot()+xlab("tr Poisson's lambda")+ylab("P(K = 4 | A)")+geom_jitter(shape=16, position=position_jitter(0.2))
#p_prob.po
