# SENSITIVITY ANALYSIS - SIM1 DATA.R
rm(list = ls())
load("~/Desktop/Small_code_R/Sim1.RData")
# SETTING:
# n = 100
# K = 2
# N = 100
...
p_RaIn.gn <- ggplot(df.RaIn.gn, aes(x=X2, y=X1)) + 
  geom_boxplot()+xlab("Gnedin's gamma")+ylab("RI Index on point est.")+ylim(0.6,1)
...
p_RaIn.po <- ggplot(df.RaIn.po, aes(x=X2, y=X1)) + 
  geom_boxplot()+xlab("tr Poisson's lambda")+ylab("RI Index on point est.")+ylim(0.6,1)
#p_RaIn.po
...
p_VaIn.gn <- ggplot(df.VaIn.gn, aes(x=X2, y=X1)) + 
  geom_boxplot()+xlab("Gnedin's gamma")+ylab("VI Index on point est.")+ylim(0, 0.6)
#p_RaIn.gn
...
p_VaIn.po <- ggplot(df.VaIn.po, aes(x=X2, y=X1)) + 
  geom_boxplot()+xlab("tr Poisson's lambda")+ylab("VI Index on point est.")+ylim(0, 0.6)
#p_RaIn.po
