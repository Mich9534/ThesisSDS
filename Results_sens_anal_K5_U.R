...
rm(list = ls())
load("~/Desktop/Small_code_R/SensAnalWhole_unbalanced.RData")
# SETTING:
# n = 400
# K = 5
# N = 100
...
p_k.gn <- ggplot(df.k_es.gn, aes(x=X2, y=X1)) + 
  geom_boxplot()+xlab("Gnedin's gamma")+ylab("K.est (K = 5)")
#p_k.gn

c1_k.po = cbind(s.a.$k_es.po[,1], rep(1, N))
c2_k.po = cbind(s.a.$k_es.po[,2], rep(6, N))
c3_k.po = cbind(s.a.$k_es.po[,3], rep(11, N))
c4_k.po = cbind(s.a.$k_es.po[,4], rep(16, N))
c5_k.po = cbind(s.a.$k_es.po[,5], rep(21, N))
...
p_k.po <- ggplot(df.k_es.po, aes(x=X2, y=X1)) + 
  geom_boxplot()+xlab("tr Poisson's lambda")+ylab("K.est (K = 5)")
#p_k.po
...
p_prob.gn <- ggplot(df.prob.gn, aes(x=X2, y=X1)) + 
  geom_boxplot()+xlab("Gnedin's gamma")+ylab("P(K = 5 | A)")+geom_jitter(shape=16, position=position_jitter(0.2))
#p_prob.gn

c1_prob.po = cbind(s.a.$prob.po[,1], rep(1, N))
c2_prob.po = cbind(s.a.$prob.po[,2], rep(6, N))
c3_prob.po = cbind(s.a.$prob.po[,3], rep(11, N))
c4_prob.po = cbind(s.a.$prob.po[,4], rep(16, N))
c5_prob.po = cbind(s.a.$prob.po[,5], rep(21, N))
...
p_prob.po <- ggplot(df.prob.po, aes(x=X2, y=X1)) + 
  geom_boxplot()+xlab("tr Poisson's lambda")+ylab("P(K = 5 | A)")+geom_jitter(shape=16, position=position_jitter(0.2))
#p_prob.po
...
p_TIME.gn <- ggplot(df.TIME.gn, aes(x=X2, y=X1)) + 
  geom_boxplot()+xlab("Gnedin's gamma")+ylab("Execution Time (min)")
#p_TIME.gn

c1_TIME.po = cbind(s.a.$TIME.po[,1], rep(1, N))
c2_TIME.po = cbind(s.a.$TIME.po[,2], rep(6, N))
c3_TIME.po = cbind(s.a.$TIME.po[,3], rep(11, N))
c4_TIME.po = cbind(s.a.$TIME.po[,4], rep(16, N))
c5_TIME.po = cbind(s.a.$TIME.po[,5], rep(21, N))
...
p_TIME.po <- ggplot(df.TIME.po, aes(x=X2, y=X1)) + 
  geom_boxplot()+xlab("tr Poisson's lambda")+ylab("Execution Time (min)")
#p_TIME.po
...
p_RaIn.gn <- ggplot(df.RaIn.gn, aes(x=X2, y=X1)) + 
  geom_boxplot()+xlab("Gnedin's gamma")+ylab("Rand Index (avg)")
#p_RaIn.gn

c1_RaIn.po = cbind(s.a.$RaIn.po[,1], rep(1, N))
c2_RaIn.po = cbind(s.a.$RaIn.po[,2], rep(6, N))
c3_RaIn.po = cbind(s.a.$RaIn.po[,3], rep(11, N))
c4_RaIn.po = cbind(s.a.$RaIn.po[,4], rep(16, N))
c5_RaIn.po = cbind(s.a.$RaIn.po[,5], rep(21, N))
...
p_RaIn.po <- ggplot(df.RaIn.po, aes(x=X2, y=X1)) + 
  geom_boxplot()+xlab("tr Poisson's lambda")+ylab("Rand Index (avg)")
#p_RaIn.po
