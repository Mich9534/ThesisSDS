...
rm(list = ls())
load("~/Desktop/Small_code_R/SensAmUnbalancedK2.RData")
# SETTING:
# n = 100
# K = 2
# N = 100
...
p_k.gn <- ggplot(df.k_es.gn, aes(x=X2, y=X1)) + 
  geom_boxplot()+xlab("Gnedin's gamma")+ylab("K.est (K = 2)")
#p_k.gn
...
p_k.po <- ggplot(df.k_es.po, aes(x=X2, y=X1)) + 
  geom_boxplot()+xlab("tr Poisson's lambda")+ylab("K.est (K = 2)")
#p_k.po
...
p_prob.gn <- ggplot(df.prob.gn, aes(x=X2, y=X1)) + 
  geom_boxplot()+xlab("Gnedin's gamma")+ylab("P(K = 2 | A)")
#p_prob.gn
...
p_prob.po <- ggplot(df.prob.po, aes(x=X2, y=X1)) + 
  geom_boxplot()+xlab("tr Poisson's lambda")+ylab("P(K = 2 | A)")
#p_prob.po
