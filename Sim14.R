# SENSITIVITY ANALYSIS - SIM14 DATA.R
rm(list = ls())
load("~/Desktop/Small_code_R/Sim14.RData")
# SETTING:
# n = 100
# K = 5
# N = 100
...
p_prob.gn <- ggplot(df.prob.gn, aes(x=X2, y=X1)) + 
  geom_boxplot()+xlab("Gnedin's gamma")+ylab("P(K = 5 | A)")
#p_prob.gn
...
p_prob.po <- ggplot(df.prob.po, aes(x=X2, y=X1)) + 
  geom_boxplot()+xlab("tr Poisson's lambda")+ylab("P(K = 5 | A)")
#p_prob.po
...
p_RaIn.gn <- ggplot(df.RaIn.gn, aes(x=X2, y=X1)) + 
  geom_boxplot()+xlab("Gnedin's gamma")+ylab("RI Index on point est.")
#p_RaIn.gn
...
p_RaIn.po <- ggplot(df.RaIn.po, aes(x=X2, y=X1)) + 
  geom_boxplot()+xlab("tr Poisson's lambda")+ylab("RI Index on point est.")
#p_RaIn.po
...
p_VaIn.gn <- ggplot(df.VaIn.gn, aes(x=X2, y=X1)) + 
  geom_boxplot()+xlab("Gnedin's gamma")+ylab("VI Index on point est.")
#p_RaIn.gn
...
p_VaIn.po <- ggplot(df.VaIn.po, aes(x=X2, y=X1)) + 
  geom_boxplot()+xlab("tr Poisson's lambda")+ylab("VI Index on point est.")
#p_RaIn.po
